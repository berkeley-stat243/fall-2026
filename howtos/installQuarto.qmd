...
## Using bash shell code chunks

If you want to use bash chunks in your document, you have a few options.

 - Use `%%bash` at the start of any chunk with bash code. This will work with the Jupyter engine, but it has the downside that all output from the cell will appear after all the code rather than the output being interspersed with the line of code that produces it.
 - Use the `knitr` engine for Quarto and tag chunks as "```{bash}". You will need to add `engine: knitr` to the YAML preface of your qmd document. See `unit2-bash.qmd for an example. Also you need  to have R installed as well as the `rmarkdown` package for R.
 - (Not recommended as it involves more steps and we haven't tested it) Install the `bash_kernel` Python package using pip or conda and install the bash kernel for Jupyter.
