...
In climate science one often wants to look at "anomalies", which are the variations in the data after shifting and scaling to a common mean and variance. Here's a closure that allows us to set up the values to transform by and then apply the transformation to any new data vector we want.
...
transform = transform_constructor(x)
del x # to demonstrate we no longer need x

data = np.random.normal(size = 10, loc = 0.2)
transform(data)
```
