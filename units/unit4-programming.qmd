...
Also note that if we just imported `mypkg3`, then `mysubpkg` is not available as it is not imported in the main `__init__.py`.

I'm not able to illustrate that here in the rendered document, but if you run the code below in
