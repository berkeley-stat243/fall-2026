...
Also note that if we just imported `mypkg`, then `mysubpkg` is not available as it is not imported in the main `__init__.py`.
For reasons unclear to me, I'm not able to illustrate that here in the rendered document, but if you run the code below in
a new Python session, you'd find that `mypkg.mysubpkg` is not available.
...
`from . import mysubpkg` to `mypkg/__init__.py`, which uses [relative imports](https://docs.python.org/3/reference/import.html). The alternative "absolute" import would be `import mypkg.mysubpkg`, which finds `mypkg` based on `sys.path` (which specifies a set of paths of where to look).
