...
We'll focus on databases/SQL and Dask in this Unit. The landscape of tools for working with large datasets is huge, so we can only cover so much here, but a main goal of the unit is to see various general concepts in specific contexts. This should set you up to understand and use those concepts with other tools/platforms.

The material on using Spark is provided for reference, but you're not responsible for that material.
...
## (BACKGROUND) An editorial on 'big data'
...
population. Consider the immense difficulties we saw in
...
It would get even worse if there were a field related to teachers for
...
One can use SQL syntax in lots of situations in which there is not a formal database set up. For example, one can use SQL syntax to query a Polars DataFrame or LazyFrame using `polars.sql`.

# 3. Recent tools and data storage formats
...
*Polars* is advertised as a very fast in-memory package for working with dataframes (i.e., an alternative to Pandas) that provides a Python interface. It uses the Arrow columnar format. It also provides a lazy execution model (using a lazy version of a DataFrame called a "LazyFrame") like Dask or Spark that allows for automatic optimization of queries.

# 4. MapReduce and Dask
...
Two commonly-used tools for doing this are Python's Dask
package and Spark. We'll cover Dask. Spark was popular for a while, but I'm
...
A similar paradigm that is implemented in `pandas`, `polars`, and `dplyr` is the
...
Note that as discussed in Unit 4 the concepts of *map* and *reduce* are core concepts in
functional programming, and of course Python provides the `map` function.

### (OPTIONAL) Hadoop and Spark
...
## Dask "schedulers" and parallelization concepts

We'll talk about the following ideas much more in Unit 6, but we need a bit of
knowledge about them here to understand how Dask divides computation on large datasets.

One sets the Dask *scheduler* to control how parallelization is done, whether
to run code on multiple machines, and how many cores on each machine to use.

The Dask schedulers are:
...
multiple processes. (You can think of threads as "lightweight" processes.)
An advantage of using the `threads` scheduler is that
the different threads can share objects in memory without making copies.

The `distributed` scheduler was originally developed for parallelizing across multiple machines (i.e., nodes in a cluster),
but it's fine and actually now recommended to use `distributed` instead of `processes` even when working on a single machine
as well. [According to the Dask documentation, it has advantages
    over the processes scheduler](https://docs.dask.org/en/stable/scheduling.html), including the diagnostic dashboard (see the
    tutorial), smarter task scheduling, and better handling of when copies need to be made. 
...
However, Dask uses delayed (lazy) evaluation. In this case, the reading is delayed until
...
By default bags are handled via the `processes` scheduler, but as noted
earlier, it's best practice to use the `distributed` scheduler.
